...
- **Android `<input type=file>` 单选、单 MIME 类型**：当前实现委托给 `ActivityResultContracts.GetContent`，并只取 `acceptTypes` 的第一个 MIME。`<input multiple>` 会被降级为单选，`accept="image/*,application/pdf"` 这种多类型会被截断为第一个。多选与多 MIME 计划在后续版本通过 `OpenMultipleDocuments` + `EXTRA_MIME_TYPES` 支持。
